...
# --- DESCRIPTION ---
# cogvlm.py is the main script that runs the cogVLM baseline in AI2Thor on the floorplans and tasks specified in the command line using the utils and definitions defined in cogvlm_utils.py
...
from vlms.cogvlm.cogvlm_utils import *
...
while not success:
    try:
        response = get_cogvlm_response(env, config, action_or_planner="planner")
...
for step_num in tqdm.trange(timeout):
    ...
    while not success:
        try:
            response = get_cogvlm_response(env, config, action_or_planner="action")
            outdict = get_action(response)
            success = True
        except:
            pass
    ...
    while not success:
        try:
            response = get_cogvlm_response(env, config, action_or_planner="verifier")
            outdict = get_action(response)
            if args.verbose:
