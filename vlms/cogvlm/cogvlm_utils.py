...
# --- DESCRIPTION ---
# This is the utils file for the cogvlm baseline that defines the environment and functions used in cogvlm.py. This script is not directly executed!

# choose appropriate device, tokenizer, and VLM model
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
tokenizer = LlamaTokenizer.from_pretrained("lmsys/vicuna-7b-v1.5")
MODEL_PATH = "THUDM/cogagent-chat-hf"

# logic to choose between torch.float16 and torch.bfloat16
if torch.cuda.is_available():
    gpu_name = torch.cuda.get_device_name(0)
    if "A100" in gpu_name or "TPU" in gpu_name:
        torch_type = torch.bfloat16
    else:
        torch_type = torch.float16
else:
    # default to float16 if no compatible hardware is detected
    torch_type = torch.float16
...
# import AI2Thor environment functions
...
def stitch_images(
    image1_path, image2_path, output_path, resize_width=500, resize_height=500
):
    # Open the images
...
def prepare_payload(env, config, module_name: str, addendum: str = ""):
    ...
    * the user prompt (which changes based on the state)
    ...
    input_by_model = model.build_conversation_input_ids(
        tokenizer, query=message, history=[], images=[stitched_image]
    )
    inputs = {
...
def get_cogvlm_response(env, config, action_or_planner, addendum: str = ""):
