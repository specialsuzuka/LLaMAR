...
# --- DESCRIPTION ---
# This is the utils file for the idefics2 baseline that defines the environment and functions used in idefics2.py. This script is not directly executed!
...
def prepare_payload(env, config, module_name: str, addendum: str = ""):
    ...
    * the user prompt (which changes based on the state)
    ...
    image_1 = load_image(image_path)
    image_path = env.get_frame(1)
    image_2 = load_image(image_path)
    messages = [
        {"role": "user",
                "content": [
                    {"type": "image"},
...
def get_idefics2_response(env, config, action_or_planner, addendum: str = ""):
