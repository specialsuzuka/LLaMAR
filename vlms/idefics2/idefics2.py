...
# --- DESCRIPTION ---
# idefics2.py is the main script that runs the idefics2 VLM baseline in AI2Thor on the floorplans and tasks specified in the command line using the utils and definitions defined in idefics2_utils.py
...
parser.add_argument("--name", type=str, default="idefics2")
...
while not success:
    try:
        response = get_idefics2_response(env, config, action_or_planner="planner")
...
# Execute VLM planner on specified task in specified floorplan:
...
for step_num in tqdm.trange(timeout):
    ...
    while not success:
        try:
            response = get_idefics2_response(env, config, action_or_planner="action")
            outdict = get_action(response)
            success = True
        except:
            pass
    ...
    while not success:
        try:
            response = get_idefics2_response(env, config, action_or_planner="verifier")
            outdict = get_action(response)
            if args.verbose:
