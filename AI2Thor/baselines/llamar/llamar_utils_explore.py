...
They can perform the following actions: ["navigate to object <object_id>", "rotate in <rotation> direction", "pick up object <object_id>", "put object on <receptacle_id>", "open object <object_id>", "close object <object_id>", "slice object <object_id>", “toggle object <object_id> on”, “toggle object <object_id> off”, "clean object <object_id>", "look up by angle <angle>", "look down by angle <angle>", “move in <translation> direction", "stay idle", "Done", "Explore"]
--- "Done" : Here "Done" is used when all the robots have completed the main task. Make sure to always say "Done" when you finish with the task.
--- "stay idle": "stay idle" is used when you want the robot to stay idle for one time step. This could be used to wait for the other robot to complete its subtask. Use it only when you think it is necessary.
--- "Explore": "Explore" is used when you want a robot to explore the environment to look for objects that relevant to the task. YOU CANNOT assign multiple agents to perform "Explore" in the same time step. You are ONLY allowed assign one agent to perform "Explore" in one time step. Use "Explore" ONLY when you think it is necessary.
...
* YOU CANNOT ASSIGN MULTIPLE AGENTS TO PERFORM "Explore" IN THE SAME TIME STEP.
