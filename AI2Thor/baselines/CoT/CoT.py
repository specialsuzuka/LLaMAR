...
# --- DESCRIPTION ---
# CoT.py is the main script that runs the CoT baseline in AI2Thor on the floorplans and tasks specified in the CoT.sh script using utils defined in CoT_utils.py
...
# parse optional arguments in the command line
...
for i in range(len(amt_tasks)):
    ...
    for fp_index in range(amt_floorplans):
        ...
        while retries < max_retries:
            try: # sometimes rare errors happen, so give each task and floorplan a few tries (max_retries)
