...
def get_agent_map_data(env):
    ...
    cam_position = env.controller.last_event.metadata["cameraPosition"]
    cam_orth_size = env.controller.last_event.metadata["cameraOrthSize"]
    pos_translator = ThorPositionTo2DFrameTranslator(
        env.controller.last_event.events[0].frame.shape,
        position_to_tuple(cam_position),
        cam_orth_size,
    )
    to_return = {
        "frame": env.controller.last_event.events[0].frame,
